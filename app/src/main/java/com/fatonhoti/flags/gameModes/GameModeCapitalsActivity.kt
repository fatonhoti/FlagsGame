...
class GameModeCapitalsActivity: AppCompatActivity(), GameMode {
    ...
    override fun checkGuess(guess: String) {
        if(guess == selectedCountry.capital) {
            correct++
    ...
    private fun updateViews() {
        ...
        for(i in 0 until choices.size) {
            if (i == randIndex) {
                ...
                buttons[i].text = selectedCountry.capital
            } else {
                buttons[i].text = choices[i].capital
            }
        }
